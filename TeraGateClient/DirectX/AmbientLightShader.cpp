...
void AmbientLightShader::render(
    ID3D11DeviceContext *deviceContext, int indexCount,
    D3DXMATRIX mtxWorld, D3DXMATRIX mtxView, D3DXMATRIX mtxProjection,
    ID3D11ShaderResourceView *texture, D3DXVECTOR3 lightDirection, D3DXVECTOR4 diffuseColor, D3DXVECTOR4 ambientColor)
{
    setShaderParameter(deviceContext, mtxWorld, mtxView, mtxProjection, texture, lightDirection, diffuseColor, ambientColor);
...
// 텍스쳐 리소스의 포인터를 받아 셰이더에 등록
// 텍스쳐는 반드시 렌더링 전에 설정되있어야 함을 주의.
void AmbientLightShader::setShaderParameter(
    ...
    D3DXVECTOR4              ambientColor)
{
    ...
    psCBuffer->ambientColor   = ambientColor;
...
void AmbientLightShader::renderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
