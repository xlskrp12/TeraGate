...
void Direct::release() 
{
    ...
    if (_depthOffStencilState)
        _depthOffStencilState->Release();
        _depthOffStencilState = NULL;
...
void Direct::readyDirectX()
{
    ...
    D3D11_DEPTH_STENCIL_DESC      depthOffStencilDesc;
    ...
    //▼레스터라이저 상태 생성
    // 레스터라이저 디스크립션 작성
    rasterizerDesc.FillMode              = D3D11_FILL_SOLID;//면 채워랏 D3D11_FILL_SOLID, 면 채우지 마랏D3D11_FILL_WIREFRAME
    rasterizerDesc.CullMode              = D3D11_CULL_BACK; //백페이스 컬링 D3D11_CULL_BACK 전면컬링 D3D11_CULL_FRONT
    rasterizerDesc.DepthClipEnable       = true;            //최대 뎁스값 넘어가면 클립
    rasterizerDesc.FrontCounterClockwise = false;           //정점들이 시계방향인 삼각형을 전면으로 간주할 것인가?
    ...
    // 2D렌더링에 쓸 desc초기화
    ZeroMemory(&depthOffStencilDesc, sizeof(depthOffStencilDesc));
    depthOffStencilDesc.DepthEnable      = false;// 2D렌더링을 위해 깊이값 false
    depthOffStencilDesc.DepthWriteMask   = D3D11_DEPTH_WRITE_MASK_ALL;
    depthOffStencilDesc.DepthFunc        = D3D11_COMPARISON_LESS;
    depthOffStencilDesc.StencilEnable    = true;
    depthOffStencilDesc.StencilReadMask  = 0xFF;
    depthOffStencilDesc.StencilWriteMask = 0xFF;
    depthOffStencilDesc.FrontFace.StencilFailOp      = D3D11_STENCIL_OP_KEEP;
    depthOffStencilDesc.FrontFace.StencilDepthFailOp = D3D11_STENCIL_OP_INCR;
    depthOffStencilDesc.FrontFace.StencilPassOp      = D3D11_STENCIL_OP_KEEP;
    depthOffStencilDesc.FrontFace.StencilFunc        = D3D11_COMPARISON_ALWAYS;
    depthOffStencilDesc.BackFace.StencilFailOp       = D3D11_STENCIL_OP_KEEP;
    depthOffStencilDesc.BackFace.StencilDepthFailOp  = D3D11_STENCIL_OP_DECR;
    depthOffStencilDesc.BackFace.StencilPassOp       = D3D11_STENCIL_OP_KEEP;
    depthOffStencilDesc.BackFace.StencilFunc         = D3D11_COMPARISON_ALWAYS;
    
    result = _device->CreateDepthStencilState(&depthOffStencilDesc, &_depthOffStencilState);
    if (FAILED(result)) MessageBox(NULL, L"ID3D11Device::CreateDepthStencilState()", L"Error", NULL);    
}
void Direct::setZBufferOn()
{
    _deviceContext->OMSetDepthStencilState(_depthStencilState, 1);
}
void Direct::setZBufferOff()
{
    _deviceContext->OMSetDepthStencilState(_depthOffStencilState, 1);
}