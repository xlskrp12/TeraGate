...
void Game::initWindow()
{
    ...
    else
    {
        ...
        _hwnd = CreateWindowEx(
            WS_EX_APPWINDOW,
            _appTitle, _appTitle,
            WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_POPUP | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_THICKFRAME,
            _rectWindow.left, _rectWindow.top,
            _rectWindow.right, _rectWindow.bottom,
            NULL, NULL, _hInstance, NULL);        
...
void Game::proc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
    switch (message)
    {
		...
		case WM_KEYDOWN:
		{
			...
			cs_packet_move_key_down *my_packet = reinterpret_cast<cs_packet_move_key_down *>(_ServerDef->send_buffer);
			...
			if (0 != w)
			{
				if (1 == w)
				{
					...
					std::cout << "move RIGHT_UP" << std::endl;
				}
				else
